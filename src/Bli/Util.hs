...
groupSchemaClauses commands = go commands ([], [], [], []) 
 where go [] xs = xs
       ...
       --       other modules
       go ((T_AssertSchema (Using modName)):xs) (types, relations, entities, clauses) = undefined
...
groupClauses commands = go commands ([], [], [], [], []) 
 where go [] xs = xs
       go ((T_AssertClause c):xs) (types,relations,entities,clauses,modules)
           = go xs (types, relations, entities, c:clauses,modules)
       go ((T_AssertSchema (Type t)):xs) (types,relations,entities,clauses,modules)
           = go xs (t:types, relations, entities, clauses, modules)
       go ((T_AssertSchema (TypeOf t ty)):xs) (types,relations,entities,clauses,modules)
           = go xs (types, relations, (t,ty):entities, clauses, modules)
       go ((T_AssertSchema (Pred isStored name argTypes dirs)):xs) (types,relations,entities,clauses,modules)
           = go xs (types, (name, argTypes):relations, entities, clauses,modules)
       go ((T_AssertSchema (Using modName)):xs) (types, relations, entities, clauses, modules)
